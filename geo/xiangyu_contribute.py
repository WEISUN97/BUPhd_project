...
class Instance(Geo):
    def __init__(
        self, struct_name: str, x: float, y: float, MIR: str, MAG: float, theta: float
    ):
        self.draw = f"""
...
class RacetrackInverse(Geo):
    def __init__(
        ...
        if theta != 0:
            raise NotImplementedError("RacetrackInverse reverse theta not implemented.")
...
class RingResonatorInverse(Geo):
    def __init__(
        ...
        waveguide_big = Waveguide(x=x0, y=y0, w=w + 2 * we, l=x_span, theta=0)
...
class FourBandWaveguide(Geo):
    def __init__(self, x1, y1, x2, y2, x3, y3, x4, y4, r, w) -> None:
        ...
        self.add(Waveguide(x_22, y_22, w, abs(y_31 - y_22), 90 if y3 > y2 else -90))
        if y3 > y2:
...
class BendWaveguideInv(Geo):
    def __init__(
        self, points: list[tuple[float, float]], r: float, w: float, we: float
    ) -> None:
        n_points = len(points)
        self.draw = ""
        if n_points < 2:
        ...
        if n_points == 2:
            x1, y1 = points[0]
            x2, y2 = points[1]
            vec = np.array([x2 - x1, y2 - y1]) / np.linalg.norm(
                np.array([x2 - x1, y2 - y1])
            )
            theta = np.arctan2(vec[1], vec[0]) * 180 / np.pi
            self.add(
                WaveguideInverse(
                    x1, y1, w, np.linalg.norm(np.array([x2 - x1, y2 - y1])), we, theta
                )
            )
        else:
            x_start = points[0][0]
            ...
            for i in range(n_points - 1):
                x1, y1 = points[i]
                x2, y2 = points[i + 1]
                vec1 = np.array([x2 - x1, y2 - y1]) / np.linalg.norm(
                    np.array([x2 - x1, y2 - y1])
                )
                    ...
                    if n_points == 2:
                        ...
                        self.add(
                            WaveguideInverse(
                                ...
                                y_start,
                                w,
                                np.linalg.norm(np.array([x2 - x_start, y2 - y_start])),
                                we,
                                theta1,
                            )
                        )
                ...
                self.add(
                    WaveguideInverse(
                        ...
                        y_start,
                        w,
                        np.linalg.norm(np.array([x2 - x_start, y2 - y_start])) - r,
                        we,
                        theta1,
                    )
                )
                x3, y3 = points[i + 2]
                vec2 = np.array([x3 - x2, y3 - y2]) / np.linalg.norm(
                    np.array([x3 - x2, y3 - y2])
                )
                ...
                if theta21 > 0:
                    self.add(NightyBandWaveguideInv(x21, y21, r, r, w, we, theta1))
                else:
                    self.add(
                        NightyBandWaveguideInv(x22, y22, r, r, w, we, theta2 + 180)
                    )
                x_start = x22
                y_start = y22
...
class BendWaveguide(Geo):
    def __init__(self, points: list[tuple[float, float]], r: float, w: float) -> None:
        ...
        if n_points == 2:
            ...
            self.add(
                Waveguide(
                    x1, y1, w, np.linalg.norm(np.array([x2 - x1, y2 - y1])), theta
                )
            )
        else:
            x_start = points[0][0]
            ...
            for i in range(n_points - 1):
                ...
                y22 = y2 + r if theta2 == 90 else y2 - r if theta2 == -90 else y2
                if theta21 > 0:
                    self.add(NightyBandWaveguide(x21, y21, r, r, w, theta1))
                else:
                    self.add(NightyBandWaveguide(x22, y22, r, r, w, theta2 + 180))
                x_start = x22
                y_start = y22
...
class TextOutline(Geo):
    def __init__(self, text: str, font_name: str, font_size: int, x, y) -> None:
        self.draw = (
            f"<{{{{{text}}}}} {{{{{font_name}}}}} {font_size} {x} {y} textOutline>"
        )
...
class GratingCouplerWaveguide(Geo):
    ...
    def __init__(
        ...
        x3, y3 = r * 1.3 * np.cos(np.deg2rad(theta_c / 2)), r * 1.3 * np.sin(
            np.deg2rad(theta_c / 2)
        )
        x4, y4 = x3, -y3
        x5, y5 = x2, -y2
        x6, y6 = x1, -y1
...
class DiskResonatorInv(Geo):
    def __init__(
        self, x, y, r, w, we, x_span, gap, layerA: int, layerB: int, shapeReso=None
    ) -> None:
        if shapeReso is not None:
            self.draw = f"{shapeReso} shapeReso\n"
        else:
